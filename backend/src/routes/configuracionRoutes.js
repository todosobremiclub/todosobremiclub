...
function isMissingRelation(err) {
  return err && err.code === '42P01'; // relation does not exist
}
...
router.get('/:clubId/config/categorias', requireAuth, requireClubAccess, async (req, res) => {
  ...
  try {
    ...
    if (isMissingRelation(e)) return res.json({ ok: true, categorias: [] });
...
router.get('/:clubId/config/tipos-gasto', requireAuth, requireClubAccess, async (req, res) => {
  ...
  try {
    ...
    if (isMissingRelation(e)) return res.json({ ok: true, tipos: [] });
...
router.post('/:clubId/config/tipos-gasto', requireAuth, requireClubAccess, async (req, res) => {
  ...
  try {
    ...
    if (isMissingRelation(e)) return res.status(500).json({ ok: false, error: 'Falta tabla tipos_gasto en la DB' });
