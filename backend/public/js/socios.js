...
// =============================
// CONTROL DE ACCESO (QR) - SOLO CON QR DEL CARNET
// Regla: autoriza si último pago es mes actual, anterior o siguiente.
// Si último pago es anterior al mes anterior => RECHAZADO.
// =============================
(() => {
  const hasAccesoUI = () => !!document.getElementById('acceso-section');
  if (!hasAccesoUI()) return;
  ...
  function ymToIndex(ym) {
    ...
    const m = s.match(/^(\d{4})-(\d{2})$/); // "YYYY-MM"
    if (!m) return null;
    const y = Number(m[1]);
  ...
  function nowIndex() {
    const d = new Date();
    return d.getFullYear() * 12 + d.getMonth(); // 0..11
  }
  ...
  function setStatus(ok, pill, msg) {
    const pillEl = document.getElementById('qrStatusPill');
  ...
  function setErr(text) {
    const el = document.getElementById('qrErr');
  ...
  function renderCard(qr, ultimoPagoYM) {
    ...
    const foto = '/img/user-placeholder.png';
    const nombreCompleto = `${String(qr.nombre || '')} ${String(qr.apellido || '')}`.trim();
    const meta = `DNI: ${String(qr.dni || '—')} · Socio Nº ${String(qr.numero || '—')}`;

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };

    const fotoEl = document.getElementById('qrFoto');
    if (fotoEl) fotoEl.src = foto;

    set('qrNombre', nombreCompleto || '—');
    set('qrMeta', meta);
    set('qrClub', String(qr.clubNombre || qr.clubId || '—'));
    set('qrAct', String(qr.actividad || '—'));
    set('qrCat', String(qr.categoria || '—'));
    set('qrUltimoPago', fmtYM(ultimoPagoYM));
  }
  ...
  function parseQrPayload(raw) {
    ...
    // En tu QR del carnet viene socioId y ultimoPago [1](https://secarsecurity-my.sharepoint.com/personal/lsardella_securion_com_ar/Documents/Archivos%20de%20chat%20de%20Microsoft%C2%A0Copilot/home_screen.dart)
    if (!obj.ultimoPago && !obj.ultimo_pago) throw new Error('El QR no trae "ultimoPago"');
    return obj;
  }

  // ---- Scanner (BarcodeDetector) + fallback manual ----
  let stream = null;
  let detector = null;
  let scanning = false;
  let lastValue = null;
  ...
  async function startCamera() {
    ...
    if (!('BarcodeDetector' in window)) {
      setErr('Este navegador no soporta cámara QR. Usá “Procesar texto” pegando el JSON del QR.');
      return;
    }
    ...
    try {
      ...
      const b1 = document.getElementById('btnQrStart');
      const b2 = document.getElementById('btnQrStop');
      if (b1) b1.disabled = true;
      if (b2) b2.disabled = false;

      setStatus(null, 'Escaneando…', 'Apuntá al QR del carnet.');
      scanLoop();
    } catch {
      setErr('No se pudo iniciar la cámara. Revisá permisos o usá el pegado manual.');
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnQrStart')?.addEventListener('click', startCamera);
    document.getElementById('btnQrStop')?.addEventListener('click', stopCamera);
    document.getElementById('btnQrClear')?.addEventListener('click', async () => {
      await stopCamera();
      clearUI();
    });
    document.getElementById('btnQrProcesar')?.addEventListener('click', async () => {
      const txt = document.getElementById('qrManual')?.value || '';
      await handleQr(txt);
    });
